...
if OUT_BUS && (success || OUT_FORCE)
    ...
    fprintf(fd, '\n----- ------- --------  --------  --------  --------  --------');
    if isOPF, fprintf(fd, '  -------  -------'); end
    NilsMatrix=[]; %just for me to get something to print to excel
    %NilsMatrix2=[];
    for i = 1:nb
        fprintf(fd, '\n%5d%7.3f%9.3f', bus(i, [BUS_I, VM, VA]));
        NilsMatrix(i,1:3) = bus(i, [BUS_I, VM, VA]);
        ...
        if ~isempty(g)
            ...
            NilsMatrix(i,4:5)=[sum(gen(g, PG)) sum(gen(g, QG))];
        else
            fprintf(fd, '      -         -  ');
            NilsMatrix(i,4:5)=[0 0];
        end
        if Pdf(i) || Qdf(i) || ~isempty(ld)
            if ~isempty(ld)
                fprintf(fd, '%10.2f*%9.2f*', Pdf(i) - sum(gen(ld, PG)), ...
                                             Qdf(i) - sum(gen(ld, QG)));
                NilsMatrix(i,6:7)=[(Pdf(i) - sum(gen(ld, PG))) (Qdf(i) - sum(gen(ld, QG)))];
            else
                fprintf(fd, '%10.2f%10.2f ', [ Pdf(i) Qdf(i) ]);
                NilsMatrix(i,6:7)=[Pdf(i) Qdf(i)];
            end
        else
            fprintf(fd, '       -         -   ');
            NilsMatrix(i,6:7)=[0 0];
        end
        if isOPF
    ...
    xlswrite('testitesti4.xlsx', NilsMatrix , 'busdata');
    fprintf(fd, '\n                        --------  --------  --------  --------');
    fprintf(fd, '\n               Total: %9.2f %9.2f %9.2f %9.2f', ...
        sum(gen(ong, PG)), sum(gen(ong, QG)), ...
        sum(Pdf(nzld)) - sum(gen(onld, PG)), ...
        sum(Qdf(nzld)) - sum(gen(onld, QG)));
    xlswrite('testitesti4.xlsx', [sum(gen(ong, PG)), sum(gen(ong, QG)), ...
        sum(Pdf(nzld)) - sum(gen(onld, PG)), ...
        sum(Qdf(nzld)) - sum(gen(onld, QG))] , 'busdata', 'D18:G18');
    fprintf(fd, '\n');
end
...
if OUT_BRANCH && (success || OUT_FORCE)
   ...
   xlswrite('testitesti4.xlsx', [   (1:nl)', branch(:, [F_BUS, T_BUS]), ...
                branch(:, [PF, QF]), branch(:, [PT, QT]), ...
                real(loss), imag(loss)], 'branchdata');
   xlswrite('testitesti4.xlsx', 'total' , 'branchdata', 'A18');
   xlswrite('testitesti4.xlsx', [ sum(real(loss)) sum(imag(loss))], 'branchdata', 'H18:I18');
    fprintf(fd, '\n                                                             --------  --------');
    fprintf(fd, '\n                                                    Total:%10.3f%10.2f', ...
